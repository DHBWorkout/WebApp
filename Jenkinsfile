pipeline {
    ...
    stages {
        ...
        stage('Run Test') {
                    ...
                    steps {
                        script {
                            sh "docker run -d --restart=always --network=dhbworkout --ip=10.24.102.16 -v dhbworkout-webapp-test-results:/app/test-report --name=dhbworkout-test 10.22.100.20:9005/dhbworkout/webapp -sh test.sh"
                        }
                    }
                    post {
                        always {
